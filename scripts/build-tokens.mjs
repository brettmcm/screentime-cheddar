// Generates every platform's tokens from tokens/cheddar.tokens.json, which is
...
const primitiveEntries = [...leaves(tokens)]
  .filter(([path]) => isPrimitive(path))
  .map(([path]) => [camel([path.slice('color.'.length)]), path])
...
function swiftColor(value) {
  const [r, g, b] = value.components.map(round)
  const a = value.alpha ?? 1
  return `CheddarColor(red: ${r}, green: ${g}, blue: ${b}, opacity: ${round(a)})`
}

function buildSwift() {
  const primitives = primitiveEntries
    .map(([name, path]) => `    public static let ${name} = ${swiftColor(at(path).$value)}`)
    .join('\n')

  const semanticProps = semanticColorEntries
    .map(([name]) => `        public let ${name}: CheddarColor`)
    .join('\n')

  const semanticInit = semanticColorEntries.map(([name]) => `            ${name}: `)

  const paletteCases = MODES.map((mode) => {
    const brandCases = BRANDS.map((brand) => {
      const values = semanticTable(mode, brand)
      const args = semanticColorEntries
        .map(
          ([name, path], index) =>
            `${semanticInit[index]}${swiftColor(resolveColor(path, mode, brand))}`,
        )
        .join(',\n')
      void values
      return `        case .${brand}:
...
${brandCases}
  ...
  const rampCases = BRANDS.map((brand) => {
    const steps = [1, 2, 3, 4, 5, 6]
      .map(
        (i) =>
          `                step${i * 100}: ${swiftColor(resolveColor(`color.semantic.brand.${i * 100}`, 'light', brand))}`,
      )
      .join(',\n')
    return `        case .${brand}:
            return Ramp(
${steps}
  ...
  const typeStyles = [...leaves(tokens)]
    ...
    .map(([path, node]) => {
      ...
      const weight =
        { 400: '.regular', 500: '.medium', 600: '.semibold', 700: '.bold' }[v.fontWeight] ??
        '.regular'
      return `    public static let ${camel([path.slice(5)])} = CheddarTextStyle(family: "${family}", size: ${fontSize}, weight: ${weight}, lineHeight: ${round(fontSize * v.lineHeight)})`
    })
    .join('\n')

  return `// Generated by scripts/build-tokens.mjs from tokens/cheddar.tokens.json.
// Do not edit by hand — edit the JSON and run \`npm run tokens:build\`.
...
${[1, 2, 3, 4, 5, 6].map((i) => `        public let step${i * 100}: CheddarColor`).join('\n')}
    }
